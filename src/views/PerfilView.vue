...
// Función dummy para cerrar sesión
const logout = () => {
  alert('Cerrando sesión...')
  // Aquí iría la lógica real: localStorage.clear(), router.push('/login'), etc.
}
</script>
...
<template>
  <div class="bg-gray-50 min-h-screen pb-24">
    <div
      class="relative bg-gradient-to-r from-pink-500 via-purple-500 to-indigo-600 pb-24 pt-6 px-6 rounded-b-[3rem] shadow-lg shadow-purple-200"
    >
      ...
      <div class="flex flex-col items-center">
        <h2 class="text-white font-black text-2xl tracking-tight">{{ user.name }}</h2>
        <p class="text-purple-100 text-sm font-medium">{{ user.email }}</p>
      </div>
    </div>

    <div class="relative -mt-16 mb-6 flex justify-center">
      <div class="relative">
        <div class="w-32 h-32 rounded-full p-1 bg-white shadow-xl">
          <img :src="user.image" class="w-full h-full rounded-full object-cover" />
        </div>
        <button
          class="absolute bottom-1 right-1 bg-gray-900 text-white w-8 h-8 rounded-full border-2 border-white flex items-center justify-center shadow-md hover:scale-110 transition active:scale-95 cursor-pointer"
        >
    ...
    <main class="max-w-md mx-auto px-4 space-y-4">
      <div class="bg-white rounded-2xl p-2 shadow-sm shadow-gray-100 border border-gray-50">
        <button class="w-full flex items-center p-3 hover:bg-gray-50 rounded-xl transition group">
          <div
        ...
        <button class="w-full flex items-center p-3 hover:bg-gray-50 rounded-xl transition group">
          <div
            class="w-10 h-10 rounded-xl bg-purple-50 text-purple-500 flex items-center justify-center mr-4 group-hover:bg-purple-100 transition"
          >
          ...
          <div
            ...
            class="bg-red-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-full mr-2 shadow-sm shadow-red-200"
          >
            {{ user.notifications }}
          </div>
          <i class="fa-solid fa-chevron-right text-gray-300 text-xs"></i>
